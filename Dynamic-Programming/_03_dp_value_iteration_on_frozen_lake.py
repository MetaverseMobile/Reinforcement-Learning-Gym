...
def value_iterate(map, state_value, gamma=0.9, *args, **kwargs):
    ...
    for _row in range(len(map)):
        for _col in range(len(map[0])):
            ...
            # v(s) = max_a ∑{s', r} p(s', r | s, a) [r + γ v(s')]
            transform_score = np.zeros([4])
            ...
            for _action in (0, 1, 2, 3):
                ...
                if (next_row, next_col) == (7, 7):
                    # if reach the goal
                    transform_score[_action] = transfer_function[_row, _col, _action] * (1 + gamma * state_value[next_row, next_col])
                else:
                    # not the goal
                    transform_score[_action] = transfer_function[_row, _col, _action] * (0 + gamma * state_value[next_row, next_col])

            new_state_value[_row, _col] = max(transform_score)

    return new_state_value


def calculate_policy(map, pi, state_value, gamma=0.9, *args, **kwargs):
    new_pi = pi.copy()
    transfer_function = build_transfer_function(map)
    ...
    for _row in range(len(map)):
        for _col in range(len(map[0])):
            ...
            state_transform_score = np.zeros([4])
            ...
            for _action in (0, 1, 2, 3):
                ...
                if (next_row, next_col) == (7, 7):
                    ...
                    # not the goal
                    reward = 0.0
                next_value = np.max(state_value[next_row, next_col])
                state_transform_score[_action] = transfer_function[_row, _col, _action] * (reward + gamma * next_value)

            best_action = int(state_transform_score.argmax(axis=0))
...
with Environment(map=map) as environment:
    ...
    while delta >= PREDICTION_THRESHOLD:
        _state_value = value_iterate(map, state_values[-1], gamma=GAMMA)
    ...
    # 2. best policy
    pi = calculate_policy(map, pi, state_values[-1])
    policy_map = ['←', '↓', '→', '↑']
