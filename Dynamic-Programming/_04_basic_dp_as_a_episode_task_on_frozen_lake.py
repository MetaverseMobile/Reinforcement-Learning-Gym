...
def predict(map, pi, action_value, *args, **kwargs):
    ...
    for _row in range(len(map)):
        for _col in range(len(map[0])):
            ...
            for _action in (0, 1, 2, 3):
                # get next state
                if _action == 0:
                    next_row, next_col = _row, max(_col - 1, 0)
                elif _action == 1:
                    next_row, next_col = min(_row + 1, len(map) - 1), _col
                elif _action == 2:
                    next_row, next_col = _row, min(_col + 1, len(map[_row]) - 1)
                else:
                    next_row, next_col = max(_row - 1, 0), _col
                    
                # fall in a hole will set back to start point
                if map[next_row][next_col] == 'H':
                    next_row, next_col = 0, 0

                if map[next_row][next_col] == 'G':
                    # if reach the goal
                    reward = 0
                else:
                    # not the goal
                ...
                new_action_value[_row, _col, _action] = transfer_function[_row, _col, _action] * (reward + next_value)

    return new_action_value


def improve(map, pi, action_value, *args, **kwargs):
    new_pi = pi.copy()
    transfer_function = build_transfer_function(map)
        ...
        for _col in range(len(map[0])):
            ...
            for _action in (0, 1, 2, 3):
                ...
                if map[next_row][next_col] == 'G':
                    ...
                    reward = -1.0
                next_value = np.max(action_value[next_row, next_col, :])
                action_transform_score[_action] = transfer_function[_row, _col, _action] * (reward + next_value)
...
with Environment(map=map) as environment:
    ...
    while epoch_delta >= PREDICTION_THRESHOLD:
        ...
        while delta >= PREDICTION_THRESHOLD:
            _action_value = predict(map, pi, action_values[-1])

            # 1.2 Update Iteration Data
            action_values += [_action_value]
            delta = min(delta, ((action_values[-1] - action_values[-2]) ** 2).sum())
