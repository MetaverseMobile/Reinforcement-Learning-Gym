...
class Perceptron(object):
    ...
    def iterate(self, x, action, target, gamma_pow, alpha=1e-3, baseline_alpha=1e-3):
        x = np.asmatrix(x)
        pmf = self.inference(x)

        delta = target - self.baseline.dot(x.T)[0, 0]

        self.baseline += baseline_alpha * delta * x
        grad = np.outer(np.eye(len(self.action_space))[action] - pmf, x)
        self.weights += alpha * gamma_pow * delta * grad
